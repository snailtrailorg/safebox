...
词表与前端 web/src/crypto/wordlist.ts 完全一致，
确保服务端生成的恢复码和前端展示格式兼容。

使用 secrets.randbelow(2048) 均匀选取 12 个词，
比前端 rejection sampling 更简洁。
"""

import secrets

# BIP39 2048-word list — 与前端 wordlist.ts 一致
BIP39_WORDS = [
    ...
    "fade", "faith", "fall", "false", "fame", "family", "famous", "fan",
    ...
    "humble", "humor", "hundred", "hunt", "hurdle", "hurry", "hurt", "husband",
...
assert len(BIP39_WORDS) == 2048, f"Expected 2048 words, got {len(BIP39_WORDS)}"


def generate_bip39_code(n_words: int = 12) -> str:
    """生成 BIP39 恢复码（n 个单词，空格分隔）。

    使用 secrets.randbelow(2048) 均匀选取，比 rejection sampling 更简洁。
    熵 = n_words * 11 bit = 132 bit (12 词)。
    """
    words = [BIP39_WORDS[secrets.randbelow(2048)] for _ in range(n_words)]
    return " ".join(words)