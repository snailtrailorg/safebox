...
| 目标 | 说明 |
| :--- | :--- |
| **合法用户逃生** | 用户丢失本地密码 + 邮箱/手机不可用时，仍能通过助记词恢复账户。助记词使用不要求验证码，避免死锁。 |
| **被盗后可拦截** | 助记词被盗后，攻击者无法在用户不知情的情况下完成数据窃取。合法用户有明确的拦截手段。 |
| **不被骚扰拖垮** | 攻击者不能通过反复触发恢复流程骚扰用户或耗尽助记词有效性。 |
...
请求：`{ mnemonic, new_local_password_hash, new_local_salt, target, value }`

服务端：
1. 按 target/value 查找用户（邮箱或手机号，用于定位账户，非发送告警邮件）。
2. 验助记词 HMAC（助记词 132bit 不可暴力枚举，不累积失败计数、不永久锁定。失败由 RateLimitMiddleware 100/h 防骚扰）。
...
6. 返回 `{ encrypted_user_key, mnemonic_salt, initiate_token }`（15min 有效）。

客户端：
- 用助记词[+Passphrase] + mnemonic_salt 派生 K。
...
2. 存旧本地密码到 `rollback_*`（local_password_hash + local_salt + local_password_version）。
3. 写正式：新 local_password_hash + local_salt + local_password_version+1。
...
```
点击加速链接 → 输入验证码 → 验证码正确
→ status=active, 清 rollback（新本地密码确认生效）
→ 可用新密码登录
```

### freeze（签名 token，无需验证码）
```
点击冻结链接 → 正式字段回滚 = rollback_*（旧本地密码 + 旧 local_password_version 恢复）
...
| 有效期 | 24h（与冷却期一致） |
| payload | `{sub: user_id, action: "accelerate"/"freeze", rc_id: mnemonic_id}` |
...
| 方法 | 路径 | 用途 |
| :--- | :--- | :--- |
| POST | /auth/recovery/initiate | 步骤 1：验助记词，返回 encrypted_user_key + initiate_token |
