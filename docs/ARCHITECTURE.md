...
| `rollback_password_version` | 旧 password_version 副本（freeze 回滚） |
| ~~failed_attempt_count~~ | 已删除（恢复码 132bit 不可暴破，无限次尝试不锁定） |
...
```
客户端:
  生成 BIP39 恢复码（12 词）+ recovery_code_salt（随机 hex）
  生成 recovery_salt + login_salt（随机 32 字节）
  ...
  上传: auth_key_hash, login_salt, kdf_settings, encrypted_user_key, recovery_salt,
        has_master_password, recovery_code(明文), recovery_code_salt
...
POST /auth/change-password { 旧 authKey, 新 authKey, 新 login_salt }
  password_version += 1
...
freeze: 回滚 authKey+login_salt+password_version = rollback_*（旧登录密码恢复）
...
### 多设备改密同步
```
设备 A 改登录密码 → password_version+1 + revoke_all_user_tokens
设备 B（其他设备）下次 /verify：
  → password_version 不符 → 409
  → 前端提示"登录密码已在别处修改，请输入恢复码[+主密码]"
  → 用户走换设备流程（重新派生 K + 设新登录密码）
```

### Google 登录
```
Google 只验证身份（ID Token），用户仍需设登录密码
注册: Google ID Token + 登录密码 + 恢复码（客户端生成）→ 同标准注册流程
...
- 恢复码永久不重生成（无月配额），无失败锁定（132bit 不可暴破）
