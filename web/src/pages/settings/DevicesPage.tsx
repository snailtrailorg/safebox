...
export function DevicesPage() {
  ...
  const load = async () => {
    try {
      ...
      setToast({ message: e.message || t("devices.loadFailed"), type: "error" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeauthorize = async (deviceId: string) => {
    ...
    try {
      ...
      setToast({ message: t("devices.revokedToast"), type: "success" });
      await load();
    } catch (e: any) {
      setToast({ message: e.message || t("devices.revokeFailed"), type: "error" });
    } finally {
      setRevoking(null);
    }
  };
  ...
  return (
    <AppLayout title={t("settings.devicesMenu")}>
      <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
        {loading ? (
          <p style={{ textAlign: "center", color: "#999" }}>{t("common.loading")}</p>
        ) : devices.length === 0 ? (
          <p style={{ textAlign: "center", color: "#999" }}>{t("devices.noDevices")}</p>
        ) : (
          devices.map((d) => (
            <section key={d.id} style={{
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.5rem" }}>
                <div>
                  <strong>{d.client_name || d.device_name || t("devices.unnamed")}</strong>
                  {d.os_name && <span style={{ marginLeft: "0.5rem", fontSize: "0.75rem", color: "#666" }}>{d.os_name}</span>}
                  {d.is_current && <span style={{ marginLeft: "0.5rem", fontSize: "0.75rem", color: "#27ae60" }}>{t("devices.currentDevice")}</span>}
                  {d.is_revoked && <span style={{ marginLeft: "0.5rem", fontSize: "0.75rem", color: "#e74c3c" }}>{t("devices.revoked")}</span>}
                </div>
                {!d.is_current && !d.is_revoked && (
                  <button
                    ...
                    {revoking === d.id ? t("common.loading") : t("devices.revoke")}
                  </button>
                )}
              </div>
              <div style={{ fontSize: "0.75rem", color: "#999", lineHeight: 1.6 }}>
                {d.last_auth_ip && <div>{t("devices.ip")}：{d.last_auth_ip}</div>}
                <div>{t("devices.lastActive")}：{fmtDate(d.last_active_at)}</div>
                <div>{t("devices.created")}：{fmtDate(d.created_at)}</div>
              </div>
            </section>
          ))
        )}
